...
- Checkout submissions go directly through `supabase.functions.invoke('submit-order')`; the Vercel `/api/orders` route was removed in phase 5.
- Browser Supabase Auth/REST/RPC/Storage requests go directly to Supabase. There is no Vercel relay in the shipping app.
...
- Direct browser Supabase access depends on `supabase.co` being reachable from the customer network. If that changes again, proxy behavior would need to be reintroduced intentionally.
